...
void main(){
    ...
    float particleDepthLinear = (-fragViewPos.z - ${GLSL_CAMERA.near}) / (${GLSL_CAMERA.far} - ${GLSL_CAMERA.near});
    float diameter = (u_radius * 2. - ${GLSL_CAMERA.near}) / (${GLSL_CAMERA.far} - ${GLSL_CAMERA.near});

    vec2 screenUv = gl_FragCoord.xy / ${GLSL_WINDOW.resolution};
    float opaqueDepth = ${GLSL_DEFERRED.getDeferredPositionDepth('screenUv')}.w;

    float l = (opaqueDepth - particleDepthLinear) / diameter;
    float softAlpha = clamp(l * 10., 0., 1.);

    float alpha = z * softAlpha;

    color = vec4(alpha * 0.1, 0., 0., 1.);
}
`)
